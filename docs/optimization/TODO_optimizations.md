...
**三包（本地已落地，2026-08-23 起同步服务器）**：短线清理 + 数据诚实 + 反脆弱闭环  
**8/21 线上复盘总账**：[`post-aug21-ops-review.md`](post-aug21-ops-review.md)（僵尸模拟仓、一日转乐观硬闸、深度池主题帽、复盘去重等，**一项一项做**）  
**未做**：冒烟；Tushare 升积分；Cursor 9b；新增强源；轨 A1/A2（盈利修正评分、拥挤 vs 景气判据）与 B2–B4（stage 归因/集中度/成本）按需另开  
...
0–4. **第一～四波（已完成代码）**  
5. **统一部署**（2026-08-23 同步三包；冒烟另议）  
6. **按 [`post-aug21-ops-review.md`](post-aug21-ops-review.md) 一项一项做**（纸面持仓 → 升乐观硬闸 → 主题帽 → 复盘去重）  
7. Tushare 升积分 / Cursor 9b / 增强源（按需）  
